...
Boards (starting with **Jira**) are the system of record for work. OffshoreHelper syncs tickets, runs AI under policy, and writes artifacts back.
...
| Direction | Data |
|-----------|------|
| Jira → OH | Issue CRUD, comments, attachment metadata, transitions |
| OH → Jira | AI summary comments, artifact attachments, custom field updates, optional transition to “AI Draft Ready” |

Conflict rule: Jira wins for human assignee and workflow state unless OH owns a dedicated AI status field.
...
Manager swipe on mobile updates OH policy for that WorkItem and may set a Jira label/`aiFirst` field.

## Attachments

- Download allowlisted types for MCP/AI context only after PII scan  
- Upload AI artifacts as Jira attachments with clear naming: `oh-ai-{jobId}-summary.md`, `oh-ai-{jobId}.patch`  
...
- OH still enforces its own RBAC; Jira access ≠ OH admin  
